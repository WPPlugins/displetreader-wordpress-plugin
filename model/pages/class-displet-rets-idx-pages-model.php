...
class DispletRetsIdxPagesModel extends DispletRetsIdxPlugin {
	...
	public static function build_model() {
		if ( !self::_is_network_admin() ) {
			...
			self::_set_user_vars();
			self::_set_user_property_ratings();
	...
	private static function determine_mobile() {
		...
		if ( $device->isMobile() ) {
			self::$_model['device']['is_mobile'] = true;
		}
		if ( $device->isTablet() ) {
			self::$_model['device']['is_tablet'] = true;
		}
	}
	...
	private static function determine_page() {
		...
		if ( function_exists( 'has_shortcode' ) && has_shortcode( $post->post_content, 'DispletListing' ) ) {
			self::$_model['is_shortcode_page'] = true;
		}
	}

	private static function determine_popups() {
		global $post;
		if ( function_exists( 'has_shortcode' ) && has_shortcode( $post->post_content, 'DispletLoginRegister' ) ) {
			self::$_model['has_login_register_form'] = true;
		}
		if ( self::$_model['is_property_details_page'] || self::$_model['is_search_results_page'] || self::$_model['is_mobile_page'] || self::$_model['is_shortcode_page'] || !empty( self::$_options['allow_sitewide_registration'] ) || !empty( self::$_options['allow_sitewide_saved_searches'] ) ) {
			self::$_model['has_login_register_popup'] = true;
		}
		if ( self::$_model['is_property_details_page'] || !empty( self::$_options['allow_sitewide_showing_requests'] ) ) {
			self::$_model['has_request_showing_popup'] = true;
		}
		if ( self::$_model['is_search_results_page'] || !empty( self::$_options['allow_sitewide_saved_searches'] ) ) {
			self::$_model['has_save_search_popup'] = true;
		}
		if ( function_exists( 'has_shortcode' ) && has_shortcode( $post->post_content, 'DispletSaveSearchRegistration' )  ) {
			self::$_model['has_save_search_registration_form'] = true;
		}
		if ( !empty( self::$_options['allow_sitewide_save_search_registrations'] ) ) {
			self::$_model['has_save_search_registration_popup'] = true;
		}
		self::_filter_popups_conditionality();
	}

	private static function _filter_popups_conditionality() {
		$conditions = array(
			'has_login_register_form',
			'has_login_register_popup',
			'has_request_showing_popup',
			'has_save_search_popup',
			'has_save_search_registration_form',
			'has_save_search_registration_popup',
		);
		foreach ( $conditions as $condition ) {
			self::$_model[ $condition ] = apply_filters( 'displetretsidx_' . $condition, self::$_model[ $condition ] );
		}
	}
	...
	private static function get_defaults() {
		self::$_model = array(
			...
			'has_login_register_form' => false,
			'has_login_register_popup' => false,
			'has_request_showing_popup' => false,
			'has_save_search_popup' => false,
			'has_save_search_registration_form' => false,
	...
	public static function get_user_property_ratings() {
		if ( !empty( self::$_model['user_ratings'] ) ) {
			return self::$_model['user_ratings'];
		}
	}
	...
	private static function maybe_build_page() {
		...
		if ( self::$_model['has_login_register_form'] || self::$_model['has_login_register_popup'] ) {
			DispletRetsIdxPopupsModel::build_login_register();
		}
		if ( self::$_model['has_save_search_registration_form'] || self::$_model['has_save_search_registration_popup'] ) {
			DispletRetsIdxPopupsModel::build_save_search_registration();
		}
	...
	private static function _set_user_property_ratings() {
		if ( self::$_model['is_property_details_page'] && self::$_model['is_canonical'] ) {
			DispletRetsIdxPropertyDetailsPageModel::set_user_property_rating();
		}
		elseif ( self::$_model['is_search_results_page'] || self::$_model['is_shortcode_page'] ) {
			self::$_model['user_ratings'] = DispletRetsIdxLeadsModel::get_property_ratings( self::$_model['current_user_id'] );
		}
	}

	private static function _set_user_vars() {
		if ( self::$_model['is_user_logged_in'] ) {
