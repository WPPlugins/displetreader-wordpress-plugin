...
class DispletRetsIdxSettingsModel extends DispletRetsIdxPlugin {
	...
	public static function get_blog_search_results_page_url( $blog_id ) {
		if ( !empty( $blog_id ) ) {
			$search_results_page_id = self::_get_blog_search_results_page_id( $blog_id );
			if ( !empty( $search_results_page_id ) ) {
				return get_blog_permalink( $blog_id, $search_results_page_id );
			}
		}
	}

	private static function _get_blog_search_results_page_id( $blog_id ) {
		if ( !empty( $blog_id ) ) {
			$options = get_blog_option( $blog_id, self::$_slugs['options']['settings'] );
			if ( !empty( $options['search_results_page_id'] ) ) {
				return $options['search_results_page_id'];
			}
		}
	}
	...
	private static function _get_email_template_options( $choices ) {
		...
		$options['email_header'] = array(
			'section' => 'email-template-options',
			'id' => 'email_header',
			'title' => 'Email Header',
			'desc' => 'Appears above the content of each outbound email to leads',
			'type' => 'textarea',
			'class' => 'html',
		);
		$options['email_footer'] = array(
			'section' => 'email-template-options',
			'id' => 'email_footer',
			'title' => 'Email Footer',
			'desc' => 'Appears beneath the content of each outbound email to leads',
			'type' => 'textarea',
			'class' => 'html',
		);
		...
		$options['email_signature'] = array(
			...
			'desc' => 'Appears at the bottom of each outbound email to users and agents, unless an agent is assigned to the user, in which case the agent\'s signature is used instead. Available data: %%site_name%% and %%site_url%%',
			'type' => 'editor',
	...
	private static function _set_options() {
		...
		$options[] = array(
			...
			'desc' => 'Enter a numeric value from 6-18.',
			'type' => 'text',
			'std' => '10',
			'class' => 'numeric',
			'min' => 6,
		...
		$options[] = array(
			'section' => 'listings',
			'id' => 'use_multiple_photos',
			'title' => 'Multiple Photos',
			'desc' => 'Check to include a slideshow of photos for each property.',
			'type' => 'checkbox',
			'std' => 1,
		);
		...
		// Property details Walk Score section
		$options[] = array(
			'section' => 'pdp-walk-score',
			'id' => 'walk_score_widget_id',
			'title' => 'Walk Score Widget ID',
			'desc' => 'Enter to display a Walk Score',
			'type' => 'text',
			'class' => 'nohtml',
			'help' => 'http://www.walkscore.com/professional/sign-up.php',
		);
		...
		$options[] = array(
			'section' => 'search-results',
			'id' => 'allow_sitewide_saved_searches',
			'title' => 'Allow Sitewide Saved Searches',
			'desc' => 'Used in some custom templates',
			'type' => 'checkbox',
			'std' => 0,
		);
		...
		$options[] = array(
			...
			'title' => 'Message',
			'type' => 'editor',
			'std' => 'When you register, you will have the ability to save your favorite properties, leave comments, and save searches to update you when new listings hit the market. Use our powerful search tool for free!',
		);
