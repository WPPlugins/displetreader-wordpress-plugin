...
class DispletRetsIdxLeadsModel extends DispletRetsIdxPlugin {
	...
	public static function get_property_rating( $user_id, $property_id ) {
		if ( !empty( $user_id ) && !empty( $property_id ) ) {
			$saved_properties = self::get_saved_properties( $user_id );
			if ( isset( $saved_properties[ $property_id ]['rating'] ) ) {
				return intval( $saved_properties[ $property_id ]['rating'] );
			}
		}
		return false;
	}

	public static function get_property_ratings( $user_id ) {
		if ( !empty( $user_id ) ) {
			$saved_properties = self::get_saved_properties( $user_id );
			if ( !empty( $saved_properties ) && is_array( $saved_properties ) ) {
				$ratings = array();
				foreach ( $saved_properties as $saved_property ) {
					if ( !empty( $saved_property['sysid'] ) && !empty( $saved_property['rating'] ) ) {
						$ratings[ $saved_property['sysid'] ] = intval( $saved_property['rating'] );
					}
				}
				return $ratings;
			}
	...
	public static function get_saved_properties( $user_id ) {
		if ( !empty( $user_id ) ) {
			return get_user_meta( $user_id, self::$_meta_keys['lead']['saved_properties'], true );
		}
		return false;
	}
