...
class DispletRetsIdxResourcesController extends DispletRetsIdxPlugin {
	...
	private static function _enqueue_google_maps_js() {
		...
		if ( ! empty( self::$_options['google_maps_key'] ) ) {
			$params['key'] = self::$_options['google_maps_key'];
		}
	...
	private static function _enqueu_intercom_io_js_for_agent_and_search_users() {
		if ( is_user_logged_in() && DispletRetsIdxAgentsModel::is_current_user_agent() ) {
			...
			self::localize_js( 'intercom-io-script.js', 'intercomSettings', apply_filters( 'displetretsidx_intercom_vars', $intercom_vars ) );
		}
	}
