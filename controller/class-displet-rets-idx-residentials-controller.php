...
class DispletRetsIdxResidentialsController extends DispletRetsIdxResidentialsModel {
	private function add_thumb_to_primary_image_for_legacy_templates( $listing ) {
		if ( empty( $listing->image_urls->primary_big ) && !empty( $listing->image_urls->primary_thumb ) ) {
			$listing->image_urls->primary_big = $listing->image_urls->primary_thumb;
		}
	}

	private function _add_user_rating( $listing ) {
		if ( !empty( $this->_model['user_ratings'] ) && is_array( $this->_model['user_ratings'] ) ) {
			if ( isset( $this->_model['user_ratings'][ $listing->id ] ) ) {
				$listing->user_rating = $this->_model['user_ratings'][ $listing->id ];
			}
		}
	}
	...
	public static function get_similar_properties( $args ) {
		...
		if ( !empty( $price ) || !empty( $square_feet ) || !empty( $subdivision ) || !empty( $zip ) ) {
			$residential_args = array();
			if ( !empty( $num_listings ) ) {
				$residential_args['num_listings'] = $num_listings;
			}
			if ( !empty( $zip ) ) {
				$residential_args['zip'] = $zip;
			}
			if ( !empty( $price ) ) {
				$price = intval( $price );
				$residential_args['min_list_price'] = intval( $price * .75 / 1000 );
				$residential_args['max_list_price'] = intval( $price * 1.25 / 1000 );
			}
			if ( !empty( $square_feet ) ) {
				$square_feet = intval( $square_feet );
				$residential_args['min_square_feet'] = intval( $square_feet * .75 );
				$residential_args['max_square_feet'] = intval( $square_feet * 1.25 );
			}
			if ( !empty( $subdivision ) ) {
				$residential_args['subdivision'] = $subdivision;
			}
			$residentials = new DispletRetsIdxResidentials( $residential_args );
			...
			if ( $count < $num_listings && !empty( $residential_args['subdivision'] ) ) {
				unset( $residential_args['subdivision'] );
				if ( !empty( $price ) ) {
					$price = intval( $price );
					$residential_args['min_list_price'] = intval( $price * .85 / 1000 );
					$residential_args['max_list_price'] = intval( $price * 1.15 / 1000 );
				}
				if ( !empty( $square_feet ) ) {
					$square_feet = intval( $square_feet );
					$residential_args['min_square_feet'] = intval( $square_feet * .85 );
					$residential_args['max_square_feet'] = intval( $square_feet * 1.15 );
				}
				$residentials = new DispletRetsIdxResidentials( $residential_args );
				$results = $residentials->get_residentials();
			...
			if ( !empty( $listings ) && is_array( $listings ) ) {
				...
				foreach ( $listings as $listing ) {
					if ( $i < $num_listings && !empty( $listing->id ) && ( empty( $id ) || $listing->id !== $id ) ) {
						$listings_by_id[ $listing->id ] = $listing;
						$i++;
					}
				}
				return array_values( $listings_by_id );
			}
		}
		return false;
	}
	...
	private function normalize_displet_listing( $listing ) {
		...
		$this->_set_relative_images( $listing );
	}

	/**
	 * Recursively replace protocols to relative for listing images.
	 *
	 * @param $listing
	 *
	 * @return void
	 */
	private function _set_relative_images( $listing ) {
		array_walk( $listing->image_urls, array( $this, 'set_relative_paths' ) );
	}

	/**
	 * Recursively replace protocols to relative.
	 *
	 * @param $arg
	 *
	 * @return mixed
	 */
	public function set_relative_paths( &$arg ) {
		if ( is_array( $arg ) ) {
			foreach( $arg as &$ar ) {
				$ar = $this->set_relative_paths( $ar );
			}
		} elseif ( is_string( $arg ) && preg_match( '/cloudfront/i', $arg ) ) {
			$arg = str_replace( array( 'http://', 'https://' ) , '//', $arg );
		}
		return $arg;
	}
	...
	private function normalize_listing( $listing, $i ) {
		...
		$this->add_thumb_to_primary_image_for_legacy_templates( $listing );
		$this->_add_user_rating( $listing );
	}
