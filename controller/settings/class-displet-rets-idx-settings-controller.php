...
class DispletRetsIdxSettingsController extends DispletRetsIdxSettingsModel {
    ...
    public static function register_default_settings() {
	    ...
	    $displetretsidx_option = apply_filters( 'displetretsidx_default_options', self::get_defaults() );
	...
	public static function get_field_values( $field, $api_key, $group = false ){
		$query_url = self::$_api_url . '/residentials/field_options?authentication_token=' . $api_key . '&field=' . $field;
		if ( $group ) {
			$query_url .= '&group_by=' . $field;
		}
		$query_args = array(
			'timeout' => 10,
			'headers' => array(
		...
		if (is_array($json_array)) {
			...
			if ( $group && !empty( $json_data->field_options ) && is_array( $json_data->field_options ) ) {
				$field_options = array();
				foreach ( $json_data->field_options as $field_option ) {
					$field_options[] = (array) $field_option;
				}
	...
	public static function update_api_website_values( $api_key, $modify_token, $email_address ) {
		...
		if ( is_array( $json_array ) && !empty( $json_array['body'] ) ) {
			...
			if ( !empty( $json_response->success ) ) {
				do_action( 'displetretsidx_post_api_authentication', array(
					'board_ids' => $json_response->board_ids,
				) );
				return $home_url;
			}
		}
	...
	public static function update_field_options( $options, $update_settings_page = false ) {
		if ( !empty( $options['displet_app_key'] ) ) {
			$field_options = array(
				...
				'city_group' => self::get_field_values( 'city,state,board_id', $options['displet_app_key'], true ),
				...
				'subdivision' => self::get_field_values( 'subdivision', $options['displet_app_key'] ),
				'zip' => self::get_field_values( 'zip', $options['displet_app_key'] ),
				'zip_group' => self::get_field_values( 'zip,board_id', $options['displet_app_key'], true ),
			);
			foreach ( $field_options as $field => $value ) {
				if ( $field !== 'city_group' && $field !== 'zip_group' ) {
					if ( is_array( $value ) ) {
