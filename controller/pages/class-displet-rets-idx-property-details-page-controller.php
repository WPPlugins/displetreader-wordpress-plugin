...
class DispletRetsIdxPropertyDetailsPageController extends DispletRetsIdxPagesController {
	...
	public static function disable_jetpack_open_graph() {
		return false;
	}

	private static function _get_base_url() {
		global $wp;
		$page_index = strpos( $wp->request, '/page/' );
		$url_suffix = $page_index !== false ? substr( $wp->request, 0, $page_index ) : $wp->request;
		return home_url( $url_suffix );
	}
	...
	public static function get_page_id() {
		...
		if ( is_array( $options ) && ! empty( $options['property_details_page_id'] ) ) {
			return intval( $options['property_details_page_id'] );
		}
	}
	...
	public static function include_open_graph_meta_tags() {
		if ( self::$_model['is_property_details_page'] && ! empty( self::$_model['listings'][0] ) ) {
			if ( ! empty( self::$_model['canonical'] ) ) {
				echo '<meta property="og:url" content="' . self::$_model['canonical'] . '"/>' . PHP_EOL;
			}
			$title = self::get_meta_title();
			if ( ! empty( $title ) ) {
				echo '<meta property="og:title" content="' . $title . '"/>' . PHP_EOL;
			}
			if ( ! empty( self::$_model['listings'][0]->image_urls->primary_big ) ) {
				echo '<meta property="og:image" content="' . self::$_model['listings'][0]->image_urls->primary_big . '"/>' . PHP_EOL;
			}
			if ( ! empty( self::$_model['listings'][0]->internet_remarks ) ) {
				echo '<meta property="og:description" content="' . self::$_model['listings'][0]->internet_remarks . '"/>' . PHP_EOL;
			}
		}
	}
	...
	public static function set_jetpack_permalink_for_property_details_page( $permalink, $id ) {
		if ( self::$_model['is_property_details_page'] && $id === self::get_page_id() ) {
			$stacks = debug_backtrace();
			if ( !empty( $stacks ) && is_array( $stacks ) ) {
				foreach ( $stacks as $stack ) {
					if ( is_array( $stack ) && $stack['function'] === 'get_permalink' && strpos( $stack['file'], 'jetpack' ) !== false ) {
						return self::set_jetpack_sharing_permalink_for_property_details_pages( $permalink );
					}
				}
			}
		}
	...
	public static function set_jetpack_sharing_permalink_for_property_details_pages( $permalink ) {
		if ( self::$_model['is_property_details_page'] ) {
			if ( ! empty( self::$_model['listings'][0]->permalink ) ) {
				return self::$_model['listings'][0]->permalink;
			}
	...
	public static function set_jetpack_sharing_title_for_property_details_pages( $title ) {
		if ( self::$_model['is_property_details_page'] ) {
			if ( ! empty( self::$_model['listings'][0]->address ) ) {
				return self::$_model['listings'][0]->address;
			}
			return '';
		}
		return $title;
