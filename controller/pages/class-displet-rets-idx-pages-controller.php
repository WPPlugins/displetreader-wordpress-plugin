...
class DispletRetsIdxPagesController extends DispletRetsIdxPagesModel {
	...
	public static function maybe_add_rel_prev_and_next() {
	...
	private static function maybe_include_templates() {
		if ( self::$_model['is_property_details_page'] ) {
			add_action( 'wp_head', array( 'DispletRetsIdxPropertyDetailsPageController', 'include_open_graph_meta_tags' ) );
			...
			add_filter( 'sharing_title', array( 'DispletRetsIdxPropertyDetailsPageController', 'set_jetpack_sharing_title_for_property_details_pages' ) );
			add_filter( 'sharing_permalink', array( 'DispletRetsIdxPropertyDetailsPageController', 'set_jetpack_sharing_permalink_for_property_details_pages' ) );
			add_filter( 'page_link', array( 'DispletRetsIdxPropertyDetailsPageController', 'set_jetpack_permalink_for_property_details_page' ), 10, 2 );
			add_filter( 'jetpack_enable_opengraph', array( 'DispletRetsIdxPropertyDetailsPageController', 'disable_jetpack_open_graph' ) );
		}
		else if ( self::$_model['is_partial_address_page'] ) {
		...
		if ( self::$_model['has_save_search_popup'] ) {
	...
	private static function maybe_prompt_for_mobile_redirect() {
		if ( self::$_model['device']['is_mobile'] && !self::$_model['is_mobile_page'] && ! empty( self::$_options['mobile_version'] ) ) {
