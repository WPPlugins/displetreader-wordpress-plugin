...
class DispletRetsIdxOptionsController extends DispletRetsIdxOptionsModel {
	public function add_field( $args = array() ) {
		...
		$idx_disabled = ( $idx  && empty( self::$_options['displet_app_key'] ) ) ? true : false;
		if ( $auth !== 1 ) {
		...
		switch ( $type ) {
			case 'text':
				echo '<input class="regular-text ' . $class . '" type="text" id="' . $id . '" name="' . $this->_model['options_slug'] . '[' . $id . ']" value="' . esc_attr( stripslashes( $value ) ) . '" />';
				echo $auth_markup . $help_markup . $description_markup;
			break;
			case 'password':
				echo '<input class="regular-text ' . $class . '" type="password" id="' . $id . '" name="' . $this->_model['options_slug'] . '[' . $id . ']" value="' . esc_attr( stripslashes( $value ) ) . '" />';
				echo $auth_markup . $help_markup . $description_markup;
			break;
			case 'multi-text':
				foreach( $choices as $slug => $label ) {
					$value = isset( self::$_options[ $id ][ $slug ] ) ? self::$_options[ $id ][ $slug ] : $std[ $slug ];
					echo '<span>' . $label . ':</span><input class="regular-text ' . $class . '" type="text" id="' . $id . '|' . $slug . '" name="' . $this->_model['options_slug'] . '[' . $id . '|' . $slug . ']" value="' . esc_attr( $value ) . '" /><br/>';
				}
				echo $auth_markup . $help_markup . $description_markup;
			break;
			case 'textarea':
				echo '<textarea class="textarea ' . $class . '" type="text" id="' . $id . '" name="' . $this->_model['options_slug'] . '[' . $id . ']" rows="5" cols="30" >' . esc_html( stripslashes( $value ) ) . '</textarea>';
				echo $auth_markup . $help_markup . $description_markup;
			break;
			case 'select':
				echo '<select id="' . $id . '" class="select ' . $class . '" name="' . $this->_model['options_slug'] . '[' . $id . ']" >';
			...
			break;
			case 'checkbox':
				echo '<input class="checkbox ' . $class . '" type="checkbox" id="' . $id . '" name="' . $this->_model['options_slug'] . '[' . $id . ']" value="1" ' . checked( $value, true, false ) . ' />';
				echo $auth_markup . $help_markup . $description_markup;
			break;
			case 'multi-checkbox':
				$i = 0;
				foreach( $choices as $slug => $label ) {
					$value = isset( self::$_options[ $id ][ $slug ] ) ? self::$_options[ $id ][ $slug ] : $std[ $slug ];
					echo '<input class="checkbox ' . $class . '" type="checkbox" id="' . $id . '|' . $slug . '" name="' . $this->_model['options_slug'] . '[' . $id .'|' . $slug . ']" value="1" ' . checked( self::$_options[ $id ][ $slug ], true, false ) . ' />' . esc_html( $label ) . '<br/>';
					$i++;
				}
				echo $auth_markup . $help_markup . $description_markup;
			break;
		...
		if ( $idx_disabled ) {
			echo '<div style="color: #f00">Please enter an API key to use this feature.</div>';
		}
	}
	...
	public function validate_callback( $input ) {
		...
		foreach ( $this->_settings as $option ) {
			switch ( $option['type'] ) {
				...
				case 'multi-checkbox':
					$checkboxarray = array();
					foreach ( (array) $option['choices'] as $slug => $label ) {
						if ( !empty( $input[ $option['id'] . '|' . $slug ] ) ) {
