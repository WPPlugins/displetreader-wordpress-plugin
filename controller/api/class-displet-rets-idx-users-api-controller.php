...
class DispletRetsIdxUsersApiController extends DispletRetsIdxApiController {
	...
	public static function create_saved_search( $user_id, $search_name, $search_criteria, $notify = true ) {
		$endpoint_url = '/users/' . $user_id . '/saved_searches';
		$query_args = array(
			'name' => $search_name,
			'search_criteria' => $search_criteria,
			'email_frequency' => !empty( $notify ) ? 1 : 0,
		);
		$json_data = parent::query( $endpoint_url, $query_args );
		if ( !empty( $json_data->id ) ) {
			return $json_data->id;
	...
	public static function create_user( $args ) {
		...
		$user_data = array(
			...
			'last_name' => $last_name,
			'phone' => $phone,
			'referred_from_url' => $referred_from_url,
			'registered_at_url' => $registered_at_url,
		);
