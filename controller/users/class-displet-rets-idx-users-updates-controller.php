		...
		if ( !empty( $api_user_id ) ) {
			...
			if ( !empty( $args['viewed_properties'] ) ) {
				...
				foreach ( $args['viewed_properties'] as $viewed_property ) {
					$property_id = DispletRetsIdxRewriteModel::get_property_id_from_url( $viewed_property['url'] );
					if ( !empty( $property_id ) ) {
						wp_schedule_single_event( time() + $args['finished_after'] + $i * 30, 'displetretsidx_register_non_api_users_with_api_cron_per_property', array( $api_user_id, $property_id ) );
					}
				}
			}
		}
