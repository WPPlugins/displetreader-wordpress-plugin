...
class DispletRetsIdxSavedPropertiesController extends DispletRetsIdxLeadsController {
	...
	public static function get_rate_property_data_attributes() {
		$listing = array(
			'address' => displetretsidx_has_address() ? displetretsidx_get_address() : false,
			'id' => displetretsidx_has_id() ? displetretsidx_get_id() : false,
			'image_urls' => array(
				'primary_big' => displetretsidx_has_photo_url() ? displetretsidx_get_photo_url() : false,
			),
			'list_price' => displetretsidx_has_price() ? displetretsidx_get_price() : false,
			'permalink' => displetretsidx_has_permalink() ? displetretsidx_get_permalink() : false,
			'square_feet' => displetretsidx_has_square_feet() ? displetretsidx_get_square_feet() : false,
			'zip' => displetretsidx_has_zip() ? displetretsidx_get_zip() : false,
		);
		return htmlentities( json_encode( array_filter( $listing ) ) );
	}
