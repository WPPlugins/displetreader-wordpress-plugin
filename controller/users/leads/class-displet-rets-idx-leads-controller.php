...
class DispletRetsIdxLeadsController extends DispletRetsIdxLeadsModel {
	...
	public static function create_new_re_search_user( $args ) {
		...
		if ( !empty( $email ) ) {
			$user_password = !empty( $password ) ? $password : wp_generate_password( 12, false );
			$user_id = self::_create_wp_user( $email, $user_password );
			...
			if ( $send_emails ) {
				...
				DispletRetsIdxEmailController::send_new_user_registration( $email, $name, $phone, $realtor, $url, $user_info->user_login, $user_password, $assigned_agent_id, $assigned_lender_id );
			}
			...
			if ( $use_cron ) {
				...
				DispletRetsIdxLeadsController::new_user_cron_jobs( $user_details, false );
			}
	...
	private static function _create_wp_user( $email, $password ) {
			...
			if ( is_wp_error( $user_id ) ) {
				if ( is_multisite() && $user_id->get_error_code() == 'existing_user_login' ) {
					...
					if ( !empty( $user ) ) {
						...
						if ( !DispletRetsIdxUtilities::in_array_of_objects( $blog_id, $blogs, 'userblog_id' ) ) {
							$success = add_user_to_blog( $blog_id, $user->ID, 'displet_user' );
	...
	public static function new_user_cron_jobs( $args, $from_cron = true ) {
		extract( $args );
		$users = array();
		if ( !empty( $user_id ) ) {
			$users[] = array(
		...
		if ( !empty( $users ) ) {
			if ( !empty( $user_registered ) ) {
				...
				if ( !empty( $time ) ) {
					$user_registered_val = get_gmt_from_date( date_i18n( 'Y-m-d H:i:s', $time ) );
					foreach ( $users as &$user ) {
						$user['user_registered'] = $user_registered_val;
					}
				}
			}
			foreach ( $users as $user ) {
		...
		$shared_meta_values = array(
			'displet_agent_id' => $assigned_agent_id,
			'displet_lender_id' => $assigned_lender_id,
			'displet_realtor' => $realtor,
			'displet_registration_url' => $url,
			'displet_registration_upstream_url' => $upstream_url,
		);
		foreach ( $shared_meta_values as $key => $value ) {
		...
		$api_user_id = DispletRetsIdxUsersApiController::create_user( array(
			...
			'last_name' => $last_name,
			'phone' => $phone,
			'referred_from_url' => $upstream_url,
			'registered_at_url' => $url,
			'username' => $username,
		) );
