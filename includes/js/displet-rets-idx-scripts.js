...
(function(displetretsidx, $, undefined) {
	...
	function ready(){
		...
		save_property_on_login();
		maybe_show_login_register_popup();
		add_save_search_popup_binding();
		init_device();
		init_login_register_form();
		init_login_register_popup();
		init_save_search_popup();
		init_save_search_registration_form();
		init_save_search_registration_popup();
		init_multi_template();
		maybe_instantiate_listings();
		do_action_from_session_storage();
	}

	function set_conditional_vars() {
		displetretsidx.has = {
			login_register_form: displetretsidx.templates.login_register_form.length ? true : false,
			login_register_popup: displetretsidx.templates.login_register_popup.length ? true : false,
			property_details_popups: displetretsidx.templates.property_details_popups.length ? true : false,
			save_search_popup: displetretsidx.templates.save_search_popup.length ? true : false,
			save_search_registration_form: displetretsidx.templates.save_search_registration_form.length ? true : false,
			save_search_registration_popup: displetretsidx.templates.save_search_registration_popup.length ? true : false,
		};
	}

	function set_defaults() {
		displetretsidx.hooks = {
			actions: [],
			filters: [],
		};
		displetretsidx.messages = {
			login_to_save_property: 'You are not logged in. Please sign in or create an account below to save properties.',
			login_to_save_search: 'You are not logged in. Please sign in or create an account below to save searches.',
		};
	}
	...
	function set_selectors() {
		...
		displetretsidx.templates = {
			login_register_form: $('[id="displet-login-register-form"]'),
			login_register_popup: $('#displet-login-register-popup'),
			property_details_popups: $('#displet-property-details-popups'),
			save_search_popup: $('#displet-save-search-popup'),
	...
	*********************/
	...
	displetretsidx.add_action = function(hook, callback) {
		displetretsidx.hooks.actions.push({
			callback: callback,
			hook: hook,
		});
	}
	...
	displetretsidx.add_filter = function(hook, callback) {
		displetretsidx.hooks.filters.push({
			callback: callback,
			hook: hook,
		});
	}
	...
	function apply_filters(hook, data) {
		if (displetretsidx.hooks && displetretsidx.hooks.filters) {
			for (var i = 0; i < displetretsidx.hooks.filters.length; i++) {
				if (displetretsidx.hooks.filters[i].hook && displetretsidx.hooks.filters[i].hook === hook && typeof(displetretsidx.hooks.filters[i].callback) === 'function') {
					return displetretsidx.hooks.filters[i].callback(data);
				}
			};
		}
		return data;
	...
	function do_action(hook, data) {
		if (displetretsidx.hooks && displetretsidx.hooks.actions) {
			for (var i = 0; i < displetretsidx.hooks.actions.length; i++) {
				if (displetretsidx.hooks.actions[i].hook && displetretsidx.hooks.actions[i].hook === hook && typeof(displetretsidx.hooks.actions[i].callback) === 'function') {
					displetretsidx.hooks.actions[i].callback(data);
				}
			};
		}
	}

	function do_action_from_session_storage() {
		if (have_session_storage_for('action_hook') && have_session_storage_for('action_data')) {
			do_action(window.sessionStorage.action_hook, JSON.parse(window.sessionStorage.action_data));
			store_in_session({
				action_data: false,
				action_hook: false,
			});
	...
	function create_tabs(scope, hashable, iteration) {
		if (iteration) {
			var tabbed = $('.displet-tabbed' + iteration, scope);
			var tabs = $('.displet-tabs' + iteration + ' a', scope);
		}
		else {
			var tabbed = $('.displet-tabbed', scope);
			var tabs = $('.displet-tabs a', scope);
		}
		if (hashable && has_hash_in(scope, tabbed)) {
			go_to_tab(window.location.hash, tabbed, tabs);
		}
		else {
			tabbed.hide().first().show();
			tabs.first().addClass('displet-active')
		}
		tabs.click(function(ev){
			...
			if (!$(this).hasClass('displet-active')) {
				...
				go_to_tab($(this).attr('href'), tabbed, tabs);
			}
			return false;
		});
		if (hashable) {
			window.onpopstate = function(event) {
				if (has_hash_in(scope, tabbed)) {
					go_to_tab(window.location.hash, tabbed, tabs);
				}
			};
		}
	...
	function has_hash_in(scope, tabbed) {
		if (window.location.hash && window.location.hash.indexOf('=') === -1 && tabbed.filter(window.location.hash).length) {
			return true;
		}
		return false;
	}
	...
	function get_cookies() {
		var cookies = {
			last_viewed_sort: '',
			last_viewed_hash: '',
			last_viewed_orientation: '',
			last_viewed_count: '',
			last_search_count: '',
			referring_site: '',
			use_mobile: '',
			user_submitted_address: '',
			user_submitted_address_time: '',
		};
		if (document.cookie) {
			var browser_cookies = document.cookie.split('; ');
			$.each(browser_cookies, function(){
				var name_value = this.split('=');
				if (name_value.length === 2 && name_value[0].indexOf('displetretsidx_') !== -1) {
					var name = name_value[0].replace('displetretsidx_', '');
					cookies[name] = decodeURIComponent( name_value[1] );
				}
			});
		}
		return cookies;
	...
	function get_listing_number_from_element(element) {
		if (element.length) {
			var classes = element.attr('class').split(/\s+/);
			if (classes.length) {
				for (var i = classes.length - 1; i >= 0; i--) {
					var match = classes[i].match(/displet-listing-([\d])+/);
	...
	function go_to_tab(id, tabbed, tabs) {
		tabbed.hide().filter(id).show();
		tabs.removeClass('displet-active').filter(function(){
			if ($(this).attr('href') === id) {
				return true;
			}
			return false;
		}).addClass('displet-active');
	}

	function have_session_storage_for(property) {
		if (window.sessionStorage && window.sessionStorage[property] && window.sessionStorage[property] !== 'false') {
			return true;
		}
		return false;
	}
	...
	function scroll_to_top_of_element(element, only_when_below){
		if (element.length) {
			var self = element.first();
			var top = self.offset().top;
			if (only_when_below) {
				var dom_top = $(document).scrollTop();
				if (dom_top > top) {
					scroll_to(top - 40);
				}
			}
	...
	function send_analytics_event(analytics_event, data) {
		if (typeof(_gaq) !== 'undefined') {
			_gaq.push(['_trackEvent', 'Displet IDX', analytics_event, data]);
		}
		else if (typeof(ga) !== 'undefined') {
			ga('send', 'event', 'Displet IDX', analytics_event, data);
		}
	}

	function send_analytics_event_from_session_storage() {
		if (typeof(Storage) !== 'undefined' && window.sessionStorage && window.sessionStorage.send_analytics_event && window.sessionStorage.analytics_event && window.sessionStorage.analytics_data) {
			send_analytics_event(window.sessionStorage.analytics_event, window.sessionStorage.analytics_data);
	...
	function send_analytics_user_data() {
		if (displetretsidx.user.is_logged_in && displetretsidx.user.login_hash && displetretsidx.user.roles) {
			if ($.inArray('administrator', displetretsidx.user.roles) !== -1) {
				var role = 'Admin';
			}
			else if ($.inArray('displet_agent', displetretsidx.user.roles) !== -1) {
				var role = 'Agent';
			}
			else if ($.inArray('displet_user', displetretsidx.user.roles) !== -1) {
				var role = 'Lead';
			}
			else {
				var role = false;
			...
			if (role) {
				if (typeof(_gaq) !== 'undefined') {
					_gaq.push(['_setCustomVar', 1, 'User Type', role, 1]);
				}
				else if (typeof(ga) !== 'undefined') {
					ga('create', 'UA-57385992-1', { 'userId': displetretsidx.user.login_hash });
	...
	function set_cookies() {
		displetretsidx.cookies = get_cookies();
		$(document).trigger('displetretsidx_have_cookies'); // Legacy usage
	}
		...
		if (typeof(Storage) !== 'undefined' && data) {
			try {
				for (property in data) {
					var value = typeof(data[property]) === 'object' ? JSON.stringify(data[property]) : data[property];
					window.sessionStorage[property] = value;
				}
			}
			catch(e) {
				console.log('Safari Private Browsing', e);
			}
		}
	}
	...
	function update_analytics() {
		send_analytics_user_data();
		send_analytics_event_from_session_storage();
	}
	...
	*********************/
	...
	function init_multi_template() {
		init_facebook_login();
		add_auto_submit_rate_property_binding('.displet-rateable-listings');
	}

	function init_facebook_login() {
		if (displetretsidx.options.facebook_app_id && displetretsidx.options.facebook_channel && (displetretsidx.has.login_register_form || displetretsidx.has.login_register_popup || displetretsidx.has.save_search_registration_form)) {
			add_facebook_js();
		}
	}

	function add_auto_submit_rate_property_binding(scope, listings) {
		var stars = $('.displet-rating', scope);
		stars.unbind('click').click(function(ev){
			ev.preventDefault();
			var listing_obj = false;
			var self = $(this);
			var rating = parseInt(self.attr('rating'));
			var ratings_container = self.parents('.displet-rate-property-auto-submit');
			var listing_attributes = ratings_container.attr('data-attr');
			if (listing_attributes) {
				var listing_obj = JSON.parse(listing_attributes);
			}
			else if (displetretsidx.pages.is_property_details_page) {
				var listing_obj = displetretsidx.property;
				var ratings_container = self.parents('.displet-rate-property-auto-submit');
			}
			else {
				var listing_id = get_listing_number_from_element(ratings_container);
				if (listing_id && listings && listings[listing_id - 1]) {
					var listing_obj = listings[listing_id - 1];
				}
			}
			if (rating && listing_obj) {
				if (displetretsidx.user.is_logged_in) {
					auto_submit_rate_property({
						listing: listing_obj,
						rating: rating,
					...
					trigger_save_property_on_login({
						listing: JSON.stringify(listing_obj),
						rating: rating,
						scope: scope,
					});
					displetretsidx.show_login_register_popup(displetretsidx.messages.login_to_save_property);
				}
			}
			return false;
		});
		stars.hover(function(){
			$(this).add($(this).prevAll()).stop(true, true).addClass('displet-active');
		}, function(){
			$(this).add($(this).prevAll()).stop(true, true).removeClass('displet-active');
		});
	}
	...
	function add_save_search_popup_binding() {
		if (displetretsidx.pages.is_search_results_page || displetretsidx.options.allow_sitewide_saved_searches) {
			$('.displet-save-search, #displet-search-form .displet-submit .displet-save').click(function(){
				var search_url = $(this).attr('displet-search-url');
				show_save_search_popup(search_url);
			});
		}
	}

	function auto_submit_rate_property(args) {
		if (args && args.rating && args.listing && args.scope) {
			var ratings_container = args.ratings_container ? args.ratings_container : $('.displet-rate-property-auto-submit', args.scope);
			ratings_container.each(function(){
				var self = $(this);
				var data = self.attr('data-attr');
				var listing = data ? JSON.parse(data) : {};
				if ((displetretsidx.pages.is_property_details_page && ( ! listing.id || ! args.listing.id )) || listing.id === args.listing.id) {
					select_rating(args.rating, self);
					self.addClass('displet-user-rated');
					submit_save_property({
						address: ( !args.listing || !args.listing.address ) && displetretsidx.pages.is_property_details_page ? displetretsidx.property.address : args.listing.address,
						image_url: ( !args.listing || !args.listing.image_urls || !args.listing.image_urls.primary_big ) && displetretsidx.pages.is_property_details_page ? displetretsidx.property.image_url : args.listing.image_urls.primary_big,
						message: '',
						price: ( !args.listing || !args.listing.list_price ) && displetretsidx.pages.is_property_details_page ? displetretsidx.property.price : args.listing.list_price,
						property_id: ( !args.listing || !args.listing.id ) && displetretsidx.pages.is_property_details_page ? displetretsidx.property.id : args.listing.id,
						property_url: ( !args.listing || !args.listing.permalink ) && displetretsidx.pages.is_property_details_page ? displetretsidx.property.permalink : args.listing.permalink,
						rating: args.rating,
						scope: self,
						square_feet: ( !args.listing || !args.listing.square_feet ) && displetretsidx.pages.is_property_details_page ? displetretsidx.property.square_feet : args.listing.square_feet,
						type: args.rating === 5 ? 'favorite' : 'possibility',
						zip: ( !args.listing || !args.listing.zip ) && displetretsidx.pages.is_property_details_page ? displetretsidx.property.zip : args.listing.zip,
					});
				}
			});
		}
	...
	function save_property_on_login() {
		if (have_session_storage_for('save_property_for_user')) {
			if (displetretsidx.user.is_logged_in) {
				if (have_session_storage_for('rating')) {
					auto_submit_rate_property({
						listing: have_session_storage_for('listing') ? JSON.parse(window.sessionStorage.listing) : false,
						rating: have_session_storage_for('rating') ? window.sessionStorage.rating : false,
						scope: have_session_storage_for('scope') ? window.sessionStorage.scope : false,
					});
				}
				else {
			...
			store_in_session({
				listing: false,
				rating: false,
				save_property_for_user: false,
				scope: false,
			});
		}
	}

	function select_rating(rating, scope){
		unselect_rating(scope);
		for (var i = 1; i <= rating; i++) {
			var star = $('.displet-rating[rating="' + i +'"]', scope);
			star.addClass('displet-on');
			if (i === rating) {
				star.addClass('displet-selected');
			}
		};
	}
	...
	function submit_save_property(args){
		$('.displet-loading', args.scope).show();
		var ajax_data = {
		...
		var public_data = {
			...
			sysid: args.property_id,
			type: args.type,
			url: args.property_url,
			zip: args.zip,
		}
		$.extend(ajax_data, public_data);
		$.post(displetretsidx.urls.ajax, ajax_data, function(response) {
			if (response == 'This property has been saved.') {
				alert(response);
				hide_save_property_popup();
				send_analytics_event('Saved Property', displetretsidx.user.email);
				do_action('after_saved_property', public_data);
			}
			else {
			...
			$('.displet-loading', args.scope).hide();
		});
	}

	function trigger_save_property_on_login(args) {
		args.save_property_for_user = true;
		store_in_session(args);
	}

	function unselect_rating(scope) {
		$('.displet-rating.displet-on', scope).removeClass('displet-on');
		$('.displet-rating.displet-selected', scope).removeClass('displet-selected');
	}
	...
	*********************/
	...
	function init_save_search_popup() {
		if (displetretsidx.has.save_search_popup) {
			set_save_search_popup_selectors();
			save_search_on_login();
			add_save_search_clients_binding();
			add_save_search_close_binding();
		}
	}

	function add_save_search_clients_binding() {
		displetretsidx.elements.save_search_popup.clients.bind('change', function(){
			if (this.value === 'new') {
				displetretsidx.elements.save_search_popup.new_client_wrapper.slideDown();
			}
			else {
				displetretsidx.elements.save_search_popup.new_client_wrapper.slideUp();
			}
		});
	}

	function add_save_search_close_binding() {
		displetretsidx.elements.save_search_popup.close.click(function(){
			hide_save_search_popup();
		});
	}

	function save_search_on_login() {
		if (have_session_storage_for('save_search_for_user')) {
			if (displetretsidx.user.is_logged_in) {
				show_save_search_popup();
			}
			store_in_session({
				save_search_for_user: false,
			});
		}
	}

	function set_save_search_popup_selectors() {
		displetretsidx.elements.save_search_popup = {
			clients: displetretsidx.templates.save_search_popup.find('.displet-client-name'),
			clients_html: displetretsidx.templates.save_search_popup.find('.displet-client-name').html(),
			close: displetretsidx.templates.save_search_popup.find('.displet-close, .displet-popup-back'),
			error_container: displetretsidx.templates.save_search_popup.find('.displet-save-search-error'),
			form: displetretsidx.templates.save_search_popup.find('.displet-form, .displet-save-search-form'),
			loading: displetretsidx.templates.save_search_popup.find('.displet-loading'),
			new_client_email: displetretsidx.templates.save_search_popup.find('.displet-new-client-email'),
			new_client_name: displetretsidx.templates.save_search_popup.find('.displet-new-client-name'),
			new_client_wrapper: displetretsidx.templates.save_search_popup.find('.displet-new-client'),
			popup_wrapper: displetretsidx.templates.save_search_popup.find('.displet-save-search-popup-wrapper'),
			search_name: displetretsidx.templates.save_search_popup.find('.displet-search-name'),
			search_notify: displetretsidx.templates.save_search_popup.find('.displet-search-notify'),
			send_emails: displetretsidx.templates.save_search_popup.find('.displet-search-send-emails'),
			submit: displetretsidx.templates.save_search_popup.find('.displet-submit'),
		};
	}

	function show_save_search_popup(search_url){
		if (displetretsidx.user.is_logged_in) {
			displetretsidx.elements.save_search_popup.submit.unbind('click').click(function(){
				submit_save_search(search_url);
			});
			displetretsidx.elements.save_search_popup.form.unbind('keypress').keypress(function(e) {
				if (e.which == 13) {
					submit_save_search(search_url);
				}
			});
			displetretsidx.elements.save_search_popup.popup_wrapper.show();
			displetretsidx.elements.html_and_body.addClass('displet-save-search-popup');
			if (displetretsidx.user.can_view_leads) {
				displetretsidx.elements.save_search_popup.loading.show();
				var data = {
				...
				$.post(displetretsidx.urls.ajax, data, function(response){
					if (response && response != 'No matching users.' && response != 'There was an error processing your request. Please try again.') {
						var markup = displetretsidx.elements.save_search_popup.clients_html;
						var clients = JSON.parse(response);
						for (var i = 0; i < clients.length; i++) {
							markup += '<option value="' + clients[i].id + '">' + clients[i].name + '</option>';
						};
						displetretsidx.elements.save_search_popup.clients.html(markup);
					}
					displetretsidx.elements.save_search_popup.loading.hide();
				});
			}
		}
		else {
			trigger_save_search_on_login();
			displetretsidx.show_login_register_popup(displetretsidx.messages.login_to_save_search);
		}
	}

	function submit_save_search(search_url){
		if (displetretsidx.has.save_search_popup) {
			displetretsidx.elements.save_search_popup.loading.show();
			var ajax_data = {
			...
			var public_data = {
				hash: search_url ? search_url.replace(displetretsidx.urls.search_results_page, '') : window.location.hash,
				search_name: displetretsidx.elements.save_search_popup.search_name.val(),
				search_notify: displetretsidx.elements.save_search_popup.search_notify.is(':checked') ? 1 : 0,
			};
			if (displetretsidx.user.can_view_leads) {
				public_data.client_id = displetretsidx.elements.save_search_popup.clients.val();
				public_data.send_emails = displetretsidx.elements.save_search_popup.send_emails.is(':checked') ? 1 : 0;
				public_data.new_client_email = displetretsidx.elements.save_search_popup.new_client_email.val();
				public_data.new_client_name = displetretsidx.elements.save_search_popup.new_client_name.val();
			}
			$.extend(ajax_data, public_data);
			$.post(displetretsidx.urls.ajax, ajax_data, function(response) {
				if (response == 'Not Logged In') {
					...
					alert(message);
					hide_save_search_popup();
					send_analytics_event('Saved Search', displetretsidx.user.email);
					do_action('after_saved_search', public_data);
				}
				else{
					displetretsidx.elements.save_search_popup.error_container.html('<div class="displet-error">' + response + '</div>');
				}
				displetretsidx.elements.save_search_popup.loading.hide();
			});
		}
	...
	function trigger_save_search_on_login() {
		store_in_session({
			save_search_for_user: true,
		});
	}
	...
	*********************/
	...
	function init_save_search_registration_form() {
		if (displetretsidx.has.save_search_registration_form) {
			set_save_search_registration_form_selectors();
			add_save_search_registration_form_facebook_login_binding();
			add_save_search_registration_form_google_login_binding();
			add_save_search_registration_form_submit_binding();
		}
	}

	function add_save_search_registration_form_facebook_login_binding() {
		displetretsidx.elements.save_search_registration_form.facebook_login.click(function(){
			displetretsidx.submit_facebook_login();
		});
	}

	function add_save_search_registration_form_google_login_binding() {
		displetretsidx.elements.save_search_registration_form.google_login.click(function(){
			submit_google_login();
		});
	}
	...
	function set_save_search_registration_form_selectors() {
		displetretsidx.elements.save_search_registration_form = {
			facebook_login: displetretsidx.templates.save_search_registration_form.find('.displet-facebook-login'),
			google_login: displetretsidx.templates.save_search_registration_form.find('.displet-google-login'),
	...
	function submit_save_search_registration_form($this_form){
		...
		$loading.show();
		var search_criteria = [];
		$.each($this_form.find('.displet-save-search-registration-form input, .displet-save-search-registration-form select'), function(){
			var value = displetretsidx.get_value_from_search_form_element(this);
			if (value) {
		...
		var public_data = {
			...
			upstream_url: decodeURIComponent(displetretsidx.cookies.referring_site),
		...
		var password_field = $this_form.find('.displet-save-search-registration-user-password');
		if (password_field.length) {
			public_data.user_password = password_field.val();
		}
		$.extend(ajax_data, public_data);
		$.post(displetretsidx.urls.ajax, ajax_data, function(response) {
			if (response === 'Saved Search' || response === 'Created User & Saved Search' || response === 'Logged In User & Saved Search') {
				$this_form.find('.displet-save-search-registration-error').html('');
				var message = 'Your search has been saved successfully.';
				alert(message);
				if (displetretsidx.elements.save_search_registration_popup) {
					hide_save_search_registration_popup();
				}
				if (response === 'Created User & Saved Search' || response === 'Logged In User & Saved Search') {
					store_in_session({
						action_data: public_data,
						action_hook: response === 'Created User & Saved Search' ? 'after_saved_search_registration' : 'after_saved_search',
						send_analytics_event: true,
						analytics_event: response === 'Created User & Saved Search' ? 'Saved Search Registration' : 'Saved Search',
						analytics_data: public_data.user_email,
					});
					window.location.reload();
				}
				else {
					send_analytics_event('Saved Search', public_data.user_email);
					do_action('after_saved_search', public_data);
				}
			}
			else{
	...
	*********************/
	...
	function init_save_search_registration_popup() {
		if (displetretsidx.has.save_search_registration_popup) {
	...
	*********************/
		...
		if ($('#displet-search-form input[name="lease_max"]').length || $('#displet-search-form input[name="sale_min"]').length) {
			...
			if (price_selects.length) {
				...
				$('#displet-search-form select[name="property_type"]').bind('change', function(){
					...
					if (!displetretsidx.wp.is_mobile_search_results_page && apply_filters('use_chosen', true)) {
	...
	displetretsidx.get_value_from_search_form_element = function(self){
		if (self.value && self.value != $(self).attr('placeholder') && ! $(self).is( ':disabled' )) {
			...
			else if ($(self).is('input[type="radio"], input[type="checkbox"]') && self.value != 'none') {
				if ($(self).is(':checked')) {
	...
	*********************/
	...
	function init_property_details_page() {
		if (displetretsidx.pages.is_property_details_page) {
			...
			add_auto_submit_rate_property_binding('#displet-property-details');
			photo_slideshow();
	...
	function add_property_details_binding(){
		...
		$('#displet-property-details .displet-thumbnails a').click(function(ev){
			ev.preventDefault();
			scroll_to_top_of_element($('#displet-property-details .displet-photo-slideshow'), true);
		});
		if (displetretsidx.wp.is_mobile_property_details_page) {
		...
		if ( displetretsidx.property.latitude && displetretsidx.property.longitude && (displetretsidx.has_road_map || displetretsidx.has_street_view_map || displetretsidx.has_birds_eye_map) ) {
			...
			if (displetretsidx.has_road_map) {
				...
				var mapOptions = {
					...
					draggable: displetretsidx.device && (displetretsidx.device.is_mobile || displetretsidx.device.is_tablet) ? false : true,
				}
			...
			if (permalinks && permalinks[window.sessionStorage.displetretsidx_search_results_page]) {
				...
				if (current_page_key !== false) {
					...
					if (page_needed && window.sessionStorage.displetretsidx_search_results_query_args) {
						...
						var url = '//api.displet.com/residentials/search?' + $.param(args) + '&callback=?';
	...
	function photo_slideshow(){
		$('#displet-property-details .displet-photo-slideshow').first().displetretsidxBxSlider({
	...
	function set_property_details_vars() {
		displetretsidx.has_birds_eye_map = displetretsidx.elements.birds_eye_map.length ? true : false;
		displetretsidx.has_road_map = displetretsidx.elements.road_map.length ? true : false;
		displetretsidx.has_street_view_map = displetretsidx.elements.street_view_map.length ? true : false;
	}
	...
	/*********************
	Property Details Page Popups Functions
	*********************/

	function init_property_details_popups() {
		if (displetretsidx.has.property_details_popups) {
			set_property_details_popups_selectors();
			add_email_friend_close_binding();
			add_email_friend_submit_binding();
			add_rating_stars_binding();
			add_rating_type_binding();
			add_save_property_close_binding();
			add_save_property_submit_binding();
		}
	}

	function add_email_friend_close_binding() {
		displetretsidx.elements.property_details_popups.email_friend_close.click(function(){
			hide_email_friend_popup();
		});
	}

	function add_email_friend_submit_binding() {
		displetretsidx.elements.property_details_popups.email_friend_submit.click(function() {
			displetretsidx.elements.property_details_popups.email_friend_loading.show();
			var ajax_data = {
			...
			var public_data = {
				...
				email: displetretsidx.elements.property_details_popups.email_friend_email.val(),
				message: displetretsidx.elements.property_details_popups.email_friend_message.val(),
				name: displetretsidx.elements.property_details_popups.email_friend_name.val(),
			...
			$.post(displetretsidx.urls.ajax, ajax_data, function(response) {
				...
				if (response == 'Your message has been sent.') {
					hide_email_friend_popup();
					send_analytics_event('Emailed Property To Friend', displetretsidx.user.email);
					do_action('after_emailed_friend', public_data);
				}
				displetretsidx.elements.property_details_popups.email_friend_loading.hide();
			});
		});
	}

	function add_rating_stars_binding() {
		displetretsidx.elements.property_details_popups.rating_stars.click(function(){
			var rating = parseInt($(this).attr('rating'));
			displetretsidx.elements.property_details_popups.rate_property_container.addClass('displet-user-rated');
			select_rating(rating, '#displet-property-details-popups .displet-save-property-popup');
		});
	}

	function add_rating_type_binding() {
		displetretsidx.elements.property_details_popups.rating_type.click(function(){
			var type = $(this).val();
			if (type === 'favorite') {
				displetretsidx.elements.property_details_popups.rate_property_container.slideDown(300);
				if (!displetretsidx.elements.property_details_popups.rate_property_container.hasClass('displet-user-rated')) {
					select_rating(3, '#displet-property-details-popups .displet-save-property-popup');
				}
			}
			else if (type === 'possibility') {
				displetretsidx.elements.property_details_popups.rate_property_container.slideDown(300);
				if (!displetretsidx.elements.property_details_popups.rate_property_container.hasClass('displet-user-rated')) {
					select_rating(2, '#displet-property-details-popups .displet-save-property-popup');
				}
			}
			else if (type === 'notes') {
				displetretsidx.elements.property_details_popups.rate_property_container.slideUp(300);
			}
		});
	}

	function add_save_property_close_binding() {
		displetretsidx.elements.property_details_popups.save_property_close.click(function() {
			hide_save_property_popup();
		});
	}

	function add_save_property_submit_binding() {
		displetretsidx.elements.property_details_popups.save_property_submit.click(function() {
			submit_save_property({
				address: displetretsidx.property.address,
				...
				property_id: displetretsidx.property.id,
				property_url: displetretsidx.property.permalink ? displetretsidx.property.permalink : window.location.href,
				rating: $('#displet-property-details-popups .displet-save-property-popup .displet-rating.displet-selected').attr('rating'),
				scope: '#displet-property-details-popups .displet-save-property-popup',
	...
	function hide_email_friend_popup() {
		displetretsidx.elements.property_details_popups.email_friend_popup_wrapper.hide();
		displetretsidx.elements.html_and_body.removeClass('displet-email-friend-popup');
	}

	function hide_save_property_popup() {
		if (displetretsidx.has.property_details_popups) {
			displetretsidx.elements.property_details_popups.save_property_popup_wrapper.hide();
			displetretsidx.elements.html_and_body.removeClass('displet-save-property-popup');
		}
	}

	function select_default_save_property_rating() {
		if (!displetretsidx.elements.property_details_popups.rate_property_container.hasClass('displet-user-rated')) {
			select_rating(3, '#displet-property-details-popups .displet-save-property-popup');
		}
	}

	function set_property_details_popups_selectors() {
		displetretsidx.elements.property_details_popups = {
			email_friend_close: displetretsidx.templates.property_details_popups.find('.displet-email-friend-popup-wrapper .displet-close, .displet-email-friend-popup-wrapper .displet-popup-back'),
			email_friend_email: displetretsidx.templates.property_details_popups.find('.displet-friend-email'),
			email_friend_loading: displetretsidx.templates.property_details_popups.find('.displet-email-friend-popup-wrapper .displet-loading'),
			email_friend_message: displetretsidx.templates.property_details_popups.find('.displet-user-message'),
			email_friend_name: displetretsidx.templates.property_details_popups.find('.displet-friend-name'),
			email_friend_popup_wrapper: displetretsidx.templates.property_details_popups.find('.displet-email-friend-popup-wrapper'),
			email_friend_submit: displetretsidx.templates.property_details_popups.find('.displet-email-friend-popup-wrapper .displet-submit'),
			rate_property_container: displetretsidx.templates.property_details_popups.find('.displet-rate-property'),
			rating_type: displetretsidx.templates.property_details_popups.find('input[name="displet-save-property-type"]'),
			rating_stars: displetretsidx.templates.property_details_popups.find('.displet-rating'),
			save_property_close: displetretsidx.templates.property_details_popups.find('.displet-save-property-popup-wrapper .displet-close, .displet-save-property-popup-wrapper .displet-popup-back'),
			save_property_popup_wrapper: displetretsidx.templates.property_details_popups.find('.displet-save-property-popup-wrapper'),
			save_property_submit: displetretsidx.templates.property_details_popups.find('.displet-save-property-popup-wrapper .displet-submit'),
		};
	}

	function show_email_friend_popup() {
		displetretsidx.elements.property_details_popups.email_friend_popup_wrapper.show();
		displetretsidx.elements.html_and_body.addClass('displet-email-friend-popup');
	}

	function show_save_property_popup() {
		if (displetretsidx.user.is_logged_in) {
			select_default_save_property_rating();
			displetretsidx.elements.property_details_popups.save_property_popup_wrapper.show();
			displetretsidx.elements.html_and_body.addClass('displet-save-property-popup');
		}
		else{
			trigger_save_property_on_login({});
			displetretsidx.show_login_register_popup(displetretsidx.messages.login_to_save_property);
	...
	******/
	...
	function init_device() {
		if (displetretsidx.user && displetretsidx.user.device && (displetretsidx.user.device.is_mobile || displetretsidx.user.device.is_tablet)) {
			set_device_selectors();
			get_geolocation();
		}
	}

	function add_nearby_listings_submit_binding() {
		displetretsidx.elements.device.nearby_listings_submit.show().click(function(ev){
			ev.preventDefault();
			submit_nearby_listings();
			return false;
		});
	...
	function detect_geolocation() {
		if (navigator && navigator.geolocation && typeof(navigator.geolocation.getCurrentPosition) === 'function') {
			navigator.geolocation.getCurrentPosition(save_geolocation);
		}
	}

	function get_geolocation() {
		if (displetretsidx.elements.device.nearby_listings_submit.length) {
			detect_geolocation();
		}
	}

	function save_geolocation(position) {
		displetretsidx.geolocation = {
			latitude: position.coords.latitude,
			longitude: position.coords.longitude,
		}
		if ( displetretsidx.elements.mobile && displetretsidx.elements.mobile.nearby_listings_loading ) {
			displetretsidx.elements.mobile.nearby_listings_loading.hide();
		}
		add_nearby_listings_submit_binding();
		do_action('saved_geolocation');
	}

	function set_device_selectors() {
		displetretsidx.elements.device = {
			nearby_listings_submit: $('.displet-submit-nearby-listings'),
		};
	}

	function submit_nearby_listings() {
		if (displetretsidx.geolocation && displetretsidx.geolocation.latitude && displetretsidx.geolocation.longitude && displetretsidx.urls && displetretsidx.urls.search_results_page) {
			var radius = .0037; // .25 miles
			var coords = [];
			for (var angle_in_degrees = 0; angle_in_degrees < 360; angle_in_degrees += 30) {
				coords.push({
					latitude: (radius * Math.cos(angle_in_degrees * Math.PI / 180)) + displetretsidx.geolocation.latitude,
					longitude: (radius * Math.sin(angle_in_degrees * Math.PI / 180)) + displetretsidx.geolocation.longitude,
				});
			};
			var url = displetretsidx.pages.is_mobile_page ? displetretsidx.urls.mobile_search_results_page : displetretsidx.urls.search_results_page;
			url += '#poly=' + coords[0].longitude + '%20' + coords[0].latitude;
	...
	******************/
	...
	function init_login_register_form() {
		if (displetretsidx.has.login_register_form || displetretsidx.has.login_register_popup) {
			set_login_register_form_selectors();
			add_login_register_form_facebook_login_binding();
			add_login_register_form_google_login_binding();
			add_login_register_form_login_toggle_binding();
			add_login_register_form_phone_binding();
			add_login_register_form_submit_login_binding();
			add_login_register_form_submit_registration_binding();
		}
	}

	function add_login_register_form_facebook_login_binding() {
		displetretsidx.elements.login_register_form.facebook_login.click(function(){
			displetretsidx.submit_facebook_login();
		});
	}

	function add_login_register_form_google_login_binding() {
		displetretsidx.elements.login_register_form.google_login.click(function(){
			submit_google_login();
		});
	}

	function add_login_register_form_login_toggle_binding() {
		displetretsidx.elements.login_register_form.login_form_toggle.click(function(){
			if (!$(this).hasClass('displet-expanded')) {
				displetretsidx.elements.login_register_form.login_form.slideDown(300);
				$(this).addClass('displet-expanded');
			}
			else{
				displetretsidx.elements.login_register_form.login_form.slideUp(300);
				$(this).removeClass('displet-expanded');
			}
		});
	}

	function add_login_register_form_phone_binding() {
		displetretsidx.elements.login_register_form.phone.mask('(999) 999-9999',{placeholder:' '});
	}

	function add_login_register_form_submit_login_binding() {
		displetretsidx.elements.login_register_form.login_submit.click(function(ev) {
			ev.preventDefault();
			submit_login_form();
			return false;
		...
		displetretsidx.elements.login_register_form.login_form.keypress(function(ev) {
			if (ev.which == 13) {
				ev.preventDefault();
				submit_login_form();
				return false;
	...
	function add_login_register_form_submit_registration_binding() {
		displetretsidx.elements.login_register_form.registration_submit.click(function(ev) {
			ev.preventDefault();
		...
		displetretsidx.elements.login_register_form.registration_form.keypress(function(ev) {
			if (ev.which == 13) {
				ev.preventDefault();
				submit_registration_form(displetretsidx.urls.current_page, displetretsidx.cookies.referring_site);
				return false;
	...
	function login_or_register_facebook_user(access_token){
		displetretsidx.elements.login_register_form.registration_loading.show();
				...
				if (user_check_response == 'User Exists and Logged In') {
					...
					$.post(displetretsidx.urls.ajax, data, function(user_registration_response) {
						if (user_registration_response == 'Successful Registration') {
							...
							displetretsidx.elements.login_register_form.registration_error.html('<div class="displet-error">' + user_registration_response + '</div>').show();
							displetretsidx.elements.login_register_form.registration_loading.hide();
						}
					});
				}
			});
	...
	function set_login_register_form_selectors() {
		var template = displetretsidx.has.login_register_form ? displetretsidx.templates.login_register_form : displetretsidx.templates.login_register_popup;
		displetretsidx.elements.login_register_form = {
			facebook_login: template.find('.displet-facebook-login'),
			google_login: template.find('.displet-google-login'),
			login_email: template.find('.displet-login-email'),
			login_error: template.find('.displet-login-error'),
			login_form: template.find('.displet-login-form'),
			login_form_toggle: template.find('.displet-login-expand-contract'),
			login_loading: template.find('.displet-login-form .displet-loading'),
			login_password: template.find('.displet-login-password'),
			login_submit: template.find('.displet-login-form .displet-submit'),
			phone: template.find('.displet-user-phone'),
			registration_email: template.find('.displet-user-email'),
			registration_error: template.find('.displet-registration-error'),
			registration_form: template.find('.displet-registration-form'),
			registration_loading: template.find('.displet-registration-form .displet-loading'),
			registration_name: template.find('.displet-user-name'),
			registration_phone: template.find('.displet-user-phone'),
			registration_realtor: template.find('.displet-user-realtor'),
			registration_submit: template.find('.displet-registration-form .displet-submit'),
		}
	}

	function submit_login_form(){
		displetretsidx.elements.login_register_form.login_loading.show();
		var login_email = displetretsidx.elements.login_register_form.login_email.val();
		var ajax_data = {
			action: 'displetretsidx_user_signon',
			_ajax_nonce: displetretsidx.nonces.login_user,
			password: displetretsidx.elements.login_register_form.login_password.val(),
		};
		var public_data = {
			email: login_email,
		};
		...
		$.post(displetretsidx.urls.ajax, ajax_data, function(response) {
			if (response == 'Successful Login') {
				store_in_session({
					action_data: public_data,
					action_hook: 'after_lead_login',
					send_analytics_event: true,
					analytics_event: 'Existing User Login',
					analytics_data: login_email,
				});
				reload_page_maybe_without_hash();
			}
			else{
				displetretsidx.elements.login_register_form.login_error.html('<div class="displet-error">' + response + '</div>').show();
				displetretsidx.elements.login_register_form.login_loading.hide();
			}
		});
	}
	...
	function submit_registration_form(current_url, current_referrer){
		displetretsidx.elements.login_register_form.registration_loading.show();
		var user_email = displetretsidx.elements.login_register_form.registration_email.val();
		var data = {
			action: 'displet_user_registration_request',
			_ajax_nonce: displetretsidx.nonces.register_user,
			listing_agent_email: displetretsidx.property.listing_agent_email,
			user_address: displetretsidx.cookies.user_submitted_address, // AustinHomeSeller.com custom functionality
			user_address_time: displetretsidx.cookies.user_submitted_address_time, // AustinHomeSeller.com custom functionality
		};
		var user_data = {
			email: user_email,
			last_hash: displetretsidx.cookies.last_viewed_hash,
			name: displetretsidx.elements.login_register_form.registration_name.val(),
			phone: displetretsidx.elements.login_register_form.registration_phone.val(),
			realtor: displetretsidx.elements.login_register_form.registration_realtor.filter(':checked').val(),
			upstream_url: decodeURIComponent(current_referrer),
			url: current_url,
		};
		...
		$.post(displetretsidx.urls.ajax, data, function(response) {
			if (response == 'Successful Registration') {
				store_in_session({
					...
					action_hook: 'after_lead_registration',
					analytics_event: 'New User Registration',
					analytics_data: user_email,
					send_analytics_event: true,
				});
				reload_page_maybe_without_hash();
			}
			else{
				displetretsidx.elements.login_register_form.registration_error.html('<div class="displet-error">' + response + '</div>').show();
				displetretsidx.elements.login_register_form.registration_loading.hide();
			}
		});
	}
	...
	*********************/
	...
	function init_login_register_popup() {
		if (displetretsidx.has.login_register_popup) {
			add_login_register_binding();
		}
	}

	function add_login_register_binding() {
		$('.displet-show-login-register-dialog, a[title="Login To View Listings"], a[title="Sign Up To View Listings"], a[title="Login To View Unlimited Listings"], a[title="Sign Up To View Unlimited Listings"], a[title="Login To Save Listings"], a[title="Sign Up To Save Listings"], a[title="Login To Save Searches"], a[title="Sign Up To Save Searches"]').click(function(e) {
			e.preventDefault();
	...
	function add_login_register_back_binding(){
		$('#displet-login-register-popup .displet-close').click(function(ev){
			ev.preventDefault();
			console.log( 'lcick' );
	...
	function hide_login_register_popup(){
		...
		displetretsidx.elements.html_and_body.removeClass('displet-user-registration-popup');
	}
	...
	displetretsidx.show_login_register_popup = function(message){
		...
		do_action( 'post_show_login_registration_popup' );
	}
	...
	*********************/
	...
	function add_request_showing_binding(){
		$('.displet-show-request-showing-dialog, a[title="Request A Showing"], a[title="Schedule A Showing"], a[title="Request Property Information"]').click(function(e){
			e.preventDefault();
			show_request_showing_popup();
			return false;
		});
	...
	function hide_request_showing_popup(){
		...
		displetretsidx.elements.html_and_body.removeClass('displet-request-showing-popup');
	}

	function show_request_showing_popup(){
		do_action( 'before_show_request_showing_dialog' );
		...
		displetretsidx.elements.html_and_body.addClass('displet-request-showing-popup');
		...
		$.post(displetretsidx.urls.ajax, ajax_data, function(response) {
			if (response === 'Sent Showing Request and Created User' || response === 'Sent Showing Request') {
				...
				if (response === 'Sent Showing Request and Created User') {
					store_in_session({
						action_data: public_data,
						action_hook: 'after_showing_request_registration',
						send_analytics_event: true,
						analytics_event: 'Showing Request Registration',
						analytics_data: public_data.user_email,
					});
					window.location.reload();
				}
					...
					hide_request_showing_popup();
					send_analytics_event('Showing Request', displetretsidx.user.email);
					do_action('after_showing_request', public_data);
				}
			}
			else {
				alert(response);
	...
	*********************/
	...
	this.displetretsidx_listings = function(args){
		...
		function create() {
			add_auto_submit_rate_property_binding(args.scope, displetretsidx.queries[self.id].listings);
			if (displetretsidx.pages.is_search_results_page) {
		...
		function add_hover_image_binding() {
			...
			self.elements.all_listings.hover(
				function(){
					...
					var y_position = $(this).offset().top - self.elements.listings_container.offset().top;
		...
		function add_multiple_photos_binding() {
			if (displetretsidx.options.use_multiple_photos) {
				if (self.slideshows) {
					$.each(self.slideshows, function(){
						if (typeof(this.destroySlider) === 'function') {
							this.destroySlider();
						}
					});
				}
				self.slideshows = [];
				$('.displet-photo-slideshow', args.scope).each(function(){
					var slideshow = $(this).find('ul').displetretsidxBxSlider({
						prevSelector: $(this).parents('.displet-listing').find('.displet-previous-photo'),
						nextSelector: $(this).parents('.displet-listing').find('.displet-next-photo'),
						pager: false,
						speed: 300,
						maxSlides: 1,
						infiniteLoop: false,
				...
				self.elements.all_listings.hover(
					function(){
						$(this).find('.displet-photo-slideshow li').each(function(){
		...
		function get_primary_photo(listing) {
			if (listing.image_urls) {
				 if (listing.image_urls.primary_big) {
				 	return listing.image_urls.primary_big;
				 }
				 if (listing.image_urls.primary_thumb) {
				 	return listing.image_urls.primary_thumb;
				 }
			}
			return false;
		}
		...
		function get_query_url() {
			if (displetretsidx.is_displet_api) {
				var url = '//api.displet.com/residentials/search?' + $.param(self.query_args) + '&callback=?';
			}
			else {
				var url = '//api.oodle.com/api/v2/listings?' + $.param(self.query_args);
			}
			console.log(url);
			return url;
		}
		...
		function has_large_image(listing) {
			if (listing && listing.image_urls && listing.image_urls.primary_big) {
				return true;
			}
			return false;
		}

		function has_large_images(listing) {
			if (listing && listing.image_urls && listing.image_urls.all_big && listing.image_urls.all_big.length > 1) {
				return true;
			}
			return false;
		}

		function has_thumbnail_image(listing) {
			if (listing && listing.image_urls && listing.image_urls.primary_thumb) {
				return true;
			}
			return false;
		}

		function has_thumbnail_images(listing) {
			if (listing && listing.image_urls && listing.image_urls.all_thumb && listing.image_urls.all_thumb.length > 1) {
				return true;
			}
			return false;
		}
		...
		function maybe_set_map() {
			if (self.is_dynamic_view) {
				if ($('#displetretsidx_canvas', args.scope).length) {
					var map = $('#displetretsidx_canvas', args.scope);
					var google_canvas = map[0];
				}
				else if ($('#map_canvas', args.scope).length) { // Legacy template usage
					var map = $('#map_canvas', args.scope);
					var google_canvas = map[0];
				}
				else {
				...
				if (google_canvas) {
					var scroll_wheel = map.attr('displet-scroll');
		...
		function set_relative_path( listing ) {

			var replacer = function( arg ) {
				for( var key in arg ) {
					if( typeof arg[key] === 'object' ) {
						replacer( arg[key] );
					} else if( typeof arg[key].replace === 'function' && /cloudfront/i.test( arg[key] ) ) {
						arg[key] = arg[key].replace( 'http://', '//' ).replace( 'https://', '//' );
					}
				}
			};

			if( typeof listing.image_urls === 'object' ) {
				replacer( listing.image_urls );
			}
		}
		...
		this.run_search_from_hash = function(is_initial_load){
			...
			if (url_hash) {
				...
				if ( self.query_args && self.query_args.search_sold && displetretsidx.user && ! displetretsidx.user.is_logged_in ) {
					displetretsidx.show_login_register_popup( 'Please sign in or register to view sold properties.' );
					return false;
				}
				if (!displetretsidx.is_displet_api && args.data_from !== 'property_showcase') {
		...
		function set_conditional_vars() {
			var map = $('.displet-map', args.scope);
			...
			self.is_map_orientation = map.length && !map.hasClass('hiding');
		...
		function set_initial_permalinks() {
			if (should_store_property_details_pagination_data()) {
				var permalinks = window.sessionStorage && window.sessionStorage.displetretsidx_search_results_permalinks ? JSON.parse(window.sessionStorage.displetretsidx_search_results_permalinks) : {};
				var the_permalinks = [];
		...
		function set_selectors() {
			...
			self.elements.listings_container = $('.displet-listings', args.scope);
		...
		this.show_map = function () {
			...
			this.elements.listings_container.addClass('hiding');
		...
		this.show_tile = function () {
			if (!displetretsidx.wp.is_mobile_search_results_page && !displetretsidx.wp.is_mobile_property_details_page) {
				...
				this.elements.listings_container.hide();
				...
				this.elements.listings_container.removeClass('hiding');
				if (get_count_from_element(this.elements.total_count) > 0) this.elements.listings_container.fadeIn(300);
				add_multiple_photos_binding();
				hovertrans();
		...
		function size_listings() {
			if (!displetretsidx.wp.is_mobile_search_results_page && !displetretsidx.wp.is_mobile_property_details_page) {
				...
				var listings_parent_width = self.elements.listings_container.width() - 1; // IE10 parent can be 621.96 px, which jQuery will call 622px, causing margins to not fit
				if (listing_width != 0) {
		...
		function update_listings() {
			...
			if (self.query_data.meta.count > 0){
				...
				if (self.is_dynamic_view) {
					if (self.is_map_orientation) {
						self.elements.map.removeClass('hiding');
					}
					else {
						self.elements.listings_container.show();
					}
				}
				var i = 1;
				$.each(self.query_data.results, function() {
					...
					update_listing_photos(this_result, this_element);
					update_listing_rating(this_result, this_element);
					...
					var populate_or_hide_fields = {
						...
						'full-baths': this_result.full_baths,
					...
					update_listing_rate_property_data(this_result, this_element);
					i++;
				});
				add_multiple_photos_binding();
				add_auto_submit_rate_property_binding(args.scope, self.query_data.results);
				if (self.is_dynamic_view) {
					hovertrans();
				}
			}
			else if (!self.query_args.poly || !self.query_args.poly.length || ! self.is_dynamic_view || ! self.is_map_orientation){
				self.elements.listings_container.stop(true, true).hide();
				if (self.is_dynamic_view) {
					if (self.is_map_orientation && !self.elements.map.hasClass('hiding') && (!self.query_args.poly || !self.query_args.poly.length)) {
		...
		function update_listing_photos(listing, element){
			if (has_large_image(listing) || has_thumbnail_image(listing)) {
				var primary_image = has_large_image(listing) ? listing.image_urls.primary_big : listing.image_urls.primary_thumb;
				element.find('div.displet-image-value').css('background-image', 'url(' + primary_image + ')');
				element.find('img.displet-image-value').attr('src', primary_image);
				element.find('.displet-image').show();
			}
			else{
				element.find('.displet-image').hide();
			}
			if (displetretsidx.options.use_multiple_photos) {
				if (has_large_images(listing) || has_thumbnail_images(listing)) {
					var images = has_large_images(listing) ? listing.image_urls.all_big : listing.image_urls.all_thumb;
					var slideshow_html = '<ul>';
					for (var i = 0; i < images.length; i++) {
						slideshow_html += '<li data-attr="' + images[i] + '"></li>';
					};
					slideshow_html += '</ul><div class="displet-previous-photo"></div><div class="displet-next-photo"></div>';
					element.find('.displet-photo-slideshow').html(slideshow_html);
				}
				else {
					element.find('.displet-photo-slideshow').html('');
				}
			}
		}

		function update_listing_rate_property_data(listing, element) {
			if (element.length) {
				var rate_property_container = element.find('.displet-rate-property-auto-submit');
				if (rate_property_container.length) {
					rate_property_container.attr('data-attr', JSON.stringify({
						address: listing.address ? listing.address : false,
						id: listing.id ? listing.id : false,
						image_urls: {
							primary_big: get_primary_photo(listing),
						},
						list_price: listing.list_price ? listing.list_price : false,
						permalink: listing.permalink ? listing.permalink : false,
						square_feet: listing.square_feet ? listing.square_feet : false,
						zip: listing.zip ? listing.zip : false,
					}));
				}
			}
		}

		function update_listing_rating(listing, element) {
			if (listing.id && displetretsidx.user.property_ratings && displetretsidx.user.property_ratings[listing.id]) {
				select_rating(displetretsidx.user.property_ratings[listing.id], element);
			}
		}
	...
	*********************/
	...
	this.displetretsidx_search = function(){
		...
		function init(){
			...
			self.elements.search_form_fields = $('#displet-search-form input[type="text"].displet-search-field, #displet-search-form select.displet-search-field, #displet-search-form input[type="radio"].displet-search-field, #displet-search-form input[type="checkbox"].displet-search-field');
			self.elements.search_sold = $('#displet-search-form input[name="search_sold"]');
			self.elements.status = $('#displet-search-form select[name="status"]');
		...
		this.update_search_form_from_hash = function(){
			if (!displetretsidx.pages.is_mobile_page) {
				this.elements.submit_search_link.each(function(){
					var self = $(this);
					self.html(self.html().replace('Submit Search', 'Revise Search'));
				});
			}
			var hash_array = window.location.hash.replace('#', '').split('/');
			var set_view = false;
		...
		this.add_search_form_binding = function(){
			...
			this.elements.search_sold.click( function() {
				if ( $( this ).is( ':checked' ) ) {
					self.elements.status.prop( 'disabled', true ).trigger( 'chosen:updated' );
				}
				else {
					self.elements.status.prop( 'disabled', false ).trigger( 'chosen:updated' );
				}
			} );
		}
