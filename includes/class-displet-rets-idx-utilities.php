...
class DispletRetsIdxUtilities {
	...
	*****************************/
	...
	public static function has_option_changed( $array_key, $original_option, $new_option ) {
		if ( !isset( $original_option[ $array_key ] ) || ( isset( $new_option[ $array_key ] ) && $new_option[ $array_key ] !== $original_option[ $array_key ] ) ) {
			return true;
		}
		return false;
	}
	...
	*****************************/
	...
	public static function double_implode( $outer_delimeter, $inner_delimter, $array ) {
		if ( !empty( $array ) && is_array( $array ) ) {
			$new_array = array();
			foreach ( $array as $key => $value) {
				$new_array[] = $key . $inner_delimter . $value;
			}
			return implode( $outer_delimeter, $new_array );
		}
		return false;
	}
