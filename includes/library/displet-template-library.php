 ...
 */
function displetretsidx_setup_template_data( $model = false ) {
	global $displetretsidx_template;
	global $displetretsidx_results;
	$displetretsidx_template = !empty( $model ) ? $model : DispletRetsIdxPagesModel::get_model();
...
function displetretsidx_the_credit() {
	...
	if ( !empty( $displetretsidx_template['options']['no_link'] ) ) {
		$credit .= 'Displet Real Estate API';
	}
	else{
		$credit .= '<a href="http://displet.com" target="_blank">Displet Real Estate API</a>';
	}
	echo $credit;
}
 ...
 */
function displetretsidx_the_show_login_register_dialog_class() {
	echo 'displet-show-login-register-dialog';
}
 ...
 * @return: boolean
 */
function displetretsidx_is_property_details_page() {
	return DispletRetsIdxPagesModel::is_property_details_page();
}
 ...
 * @return: print: CSS classname
 */
function displetretsidx_the_rateable_listings_class() {
	echo displetretsidx_get_rateable_listings_class();
}

/**
 * @return: CSS classname
 */
function displetretsidx_get_rateable_listings_class() {
	return 'displet-rateable-listings';
}
 ...
 * @return: url
 */
function displetretsidx_has_permalink() {
	global $displetretsidx_listing;
	if ( !empty( $displetretsidx_listing->permalink ) ) {
		return true;
	}
	return false;
}
 ...
 * @return: url
 */
function displetretsidx_get_permalink() {
	global $displetretsidx_listing;
	return $displetretsidx_listing->permalink;
}
...
function displetretsidx_has_photo_url() {
	global $displetretsidx_listing;
	if ( !empty( $displetretsidx_listing->image_urls->primary_big ) || !empty( $displetretsidx_listing->image_urls->primary_thumb ) ) {
		return true;
	}
	return false;
}
...
function displetretsidx_get_photo_url() {
	...
	elseif ( !empty( $displetretsidx_listing->image_urls->primary_thumb ) ) {
		return $displetretsidx_listing->image_urls->primary_thumb;
	}
}
...
function displetretsidx_the_photo_url() {
	...
	elseif ( !empty( $displetretsidx_listing->image_urls->primary_thumb ) ) {
		echo $displetretsidx_listing->image_urls->primary_thumb;
	}
}
...
function displetretsidx_the_photo_style() {
	...
	elseif ( !empty( $displetretsidx_listing->image_urls->primary_thumb ) ) {
		echo 'background-image: url( ' . $displetretsidx_listing->image_urls->primary_thumb . ' );';
	}
}
...
function displetretsidx_the_photo_container_class() {
	...
	global $displetretsidx_listing;
	$class = 'displet-image';
	if ( !empty( $displetretsidx_template['options']['use_multiple_photos'] ) ) {
		$class .= ' displet-photo-slideshow';
	}
	if ( !displetretsidx_has_photo_url() ) {
		$class .= ' displet-none';
	}
	echo $class;
}
 ...
 * @return: boolean
 */
function displetretsidx_use_multiple_photos() {
	global $displetretsidx_template;
	if ( !empty( $displetretsidx_template['options']['use_multiple_photos'] ) ) {
		return true;
		/* Shouldn't exist
		$photos = displetretsidx_get_photo_urls();
		if ( !empty( $photos ) && is_array( $photos ) && count( $photos ) > 1 ) {
			return true;
		}
		*/
	}
 ...
 * @return: print: HTML markup
 */
function displetretsidx_the_multiple_photos_slideshow( $include_links = true ) {
	$photos = displetretsidx_get_photo_urls();
	if ( !empty( $photos ) && is_array( $photos ) ) {
		$html = '<ul>' . PHP_EOL;
		foreach ( $photos as $photo ) {
			$html .= '<li data-attr="' . $photo . '"></li>' . PHP_EOL;
		}
		$html .= '</ul>' . PHP_EOL;
		if ( $include_links ) {
			$html .=
				'<div class="displet-previous-photo"></div>' . PHP_EOL .
				'<div class="displet-next-photo"></div>';
		}
		echo $html;
	}
}
 ...
 * @return: print: CSS classname
 */
function displetretsidx_the_full_baths_class() {
	echo 'displet-full-baths-value';
}
 ...
 * @return: print: CSS classname
 */
function displetretsidx_the_full_baths_container_class() {
	global $displetretsidx_listing;
	$class = 'displet-full-baths';
...
function displetretsidx_the_total_pages() {
	global $displetretsidx_results;
	if ( !empty( $displetretsidx_results['meta']->limit ) ) {
