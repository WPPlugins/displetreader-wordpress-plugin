 ...
 * @return: string
 */
function displetretsidx_get_blog_search_results_page_url( $blog_id ) {
	return DispletRetsIdxSettingsModel::get_blog_search_results_page_url( $blog_id );
}
...
/**
 * @param: $field: string or array ( area_mls_defined, city, city_group, property_type, school_district, status, subdivision, zip or zip_group )
...
function displetretsidx_get_field_options( $field ) {
	...
	if ( is_array( $field ) ) {
		...
		foreach ( $field as $slug ) {
			if ( !empty( $field_option[ $slug ] ) && is_array( $field_option[ $slug ] ) ) {
...
/**
 * @param: $field: individual field returned from array of fields using displetretsidx_get_search_form_fields() or displetretsidx_get_advanced_search_form_fields()
 * @return: HTML markup
 */
function displetretsidx_get_search_form_field_html( $field ) {
	if ( !empty( $field ) && is_array( $field ) ) {
		return DispletRetsIdxSearchFieldsController::get_search_field_markup( $field );
	}
}
 ...
 */
function displetretsidx_the_search_form_field_html( $field ) {
	echo displetretsidx_get_search_form_field_html( $field );
}
